...
function test_task16_readiness_evidence(root::AbstractString, fixture)
    @testset "Task 16 local readiness is non-certifying" begin
        ...
        @test occursin("mpi_setup_command=", commands)
        @test occursin("Pkg.develop(PackageSpec(path=pwd())); Pkg.instantiate()", commands)
        @test occursin("/Users/subha/.julia/packages/MPI/pvbg6/bin/mpiexecjl --project=test/gpu/cuda -n 4", commands)
