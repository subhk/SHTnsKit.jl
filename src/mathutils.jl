...
"""
function _loggamma(n::Integer)
    # Convert to Int for consistency
    ni = Int(n)
    
    # Validate input (gamma function requires positive arguments for integers)
    ni >= 1 || throw(DomainError(n, "_loggamma expects n ≥ 1 for Integer inputs"))
...
function _loggamma(x::Real)
    ...
    isinteger(x) && return _loggamma(Int(round(x)))
    
    # Reject non-integer reals with helpful error message
    throw(ArgumentError("_loggamma(::Real) is only defined for integers here; add SpecialFunctions for general inputs"))
end
