...
static double *copy_real_input(const double *snapshot, size_t count) {
    double *work = malloc(count * sizeof(*work));
    if (!work) { fputs("analysis input allocation failed\n", stderr); exit(1); }
    memcpy(work, snapshot, count * sizeof(*work));
    return work;
}

static cplx *copy_complex_input(const cplx *snapshot, size_t count) {
    cplx *work = malloc(count * sizeof(*work));
    if (!work) { fputs("analysis input allocation failed\n", stderr); exit(1); }
    memcpy(work, snapshot, count * sizeof(*work));
    return work;
}

/* SHTns analysis entry points accept mutable spatial arrays and may use them
   as scratch.  Never pass the immutable payload snapshot itself. */
static void analysis_spat_to_SH(shtns_cfg c, const double *input, size_t count, cplx *q) {
    double *input_work = copy_real_input(input, count);
    spat_to_SH(c, input_work, q);
    free(input_work);
}
static void analysis_spat_cplx_to_SH(shtns_cfg c, const cplx *input, size_t count, cplx *q) {
    cplx *input_work = copy_complex_input(input, count);
    spat_cplx_to_SH(c, input_work, q);
    free(input_work);
}
static void analysis_spat_to_SH_l(shtns_cfg c, const double *input, size_t count,
                                  cplx *q, int ltr) {
    double *input_work = copy_real_input(input, count);
    spat_to_SH_l(c, input_work, q, ltr);
    free(input_work);
}
static void analysis_spat_to_SH_ml(shtns_cfg c, int im, const cplx *input,
                                   size_t count, cplx *q, int ltr) {
    cplx *input_work = copy_complex_input(input, count);
    spat_to_SH_ml(c, im, input_work, q, ltr);
    free(input_work);
}
static void analysis_spat_to_SHsphtor(shtns_cfg c, const double *vt, const double *vp,
                                      size_t count, cplx *s, cplx *t) {
    double *vt_work = copy_real_input(vt, count);
    double *vp_work = copy_real_input(vp, count);
    spat_to_SHsphtor(c, vt_work, vp_work, s, t);
    free(vt_work); free(vp_work);
}
static void analysis_spat_to_SHsphtor_l(shtns_cfg c, const double *vt, const double *vp,
                                        size_t count, cplx *s, cplx *t, int ltr) {
    double *vt_work = copy_real_input(vt, count);
    double *vp_work = copy_real_input(vp, count);
    spat_to_SHsphtor_l(c, vt_work, vp_work, s, t, ltr);
    free(vt_work); free(vp_work);
}
static void analysis_spat_to_SHsphtor_ml(shtns_cfg c, int im, const cplx *vt,
                                         const cplx *vp, size_t count, cplx *s,
                                         cplx *t, int ltr) {
    cplx *vt_work = copy_complex_input(vt, count);
    cplx *vp_work = copy_complex_input(vp, count);
    spat_to_SHsphtor_ml(c, im, vt_work, vp_work, s, t, ltr);
    free(vt_work); free(vp_work);
}
static void analysis_spat_to_SHqst(shtns_cfg c, const double *vr, const double *vt,
                                   const double *vp, size_t count, cplx *q,
                                   cplx *s, cplx *t) {
    double *vr_work = copy_real_input(vr, count);
    double *vt_work = copy_real_input(vt, count);
    double *vp_work = copy_real_input(vp, count);
    spat_to_SHqst(c, vr_work, vt_work, vp_work, q, s, t);
    free(vr_work); free(vt_work); free(vp_work);
}
static void analysis_spat_to_SHqst_l(shtns_cfg c, const double *vr, const double *vt,
                                     const double *vp, size_t count, cplx *q,
                                     cplx *s, cplx *t, int ltr) {
    double *vr_work = copy_real_input(vr, count);
    double *vt_work = copy_real_input(vt, count);
    double *vp_work = copy_real_input(vp, count);
    spat_to_SHqst_l(c, vr_work, vt_work, vp_work, q, s, t, ltr);
    free(vr_work); free(vt_work); free(vp_work);
}
static void analysis_spat_to_SHqst_ml(shtns_cfg c, int im, const cplx *vr,
                                      const cplx *vt, const cplx *vp, size_t count,
                                      cplx *q, cplx *s, cplx *t, int ltr) {
    cplx *vr_work = copy_complex_input(vr, count);
    cplx *vt_work = copy_complex_input(vt, count);
    cplx *vp_work = copy_complex_input(vp, count);
    spat_to_SHqst_ml(c, im, vr_work, vt_work, vp_work, q, s, t, ltr);
    free(vr_work); free(vt_work); free(vp_work);
}
...
static void generate_analysis_family(FILE *m,const char *out) {
    ...
    { shtns_cfg c=make_cfg(lmax,lmax,1,sht_orthonormal,sht_gauss,nlat,nphi,SHT_PHI_CONTIGUOUS);double *v=calloc((size_t)nsp*batch,sizeof(*v)),*tmp=calloc(nsp,sizeof(*tmp));cplx *q=calloc((size_t)c->nlm*batch,sizeof(*q));
      for(int k=0;k<batch;++k){fill_spatial_real(tmp,nsp,0.2+0.1*k);analysis_spat_to_SH(c,tmp,nsp,q+(size_t)k*c->nlm);for(int ip=0;ip<nphi;++ip)for(int it=0;it<nlat;++it)v[it+(size_t)nlat*ip+(size_t)nsp*k]=tmp[it*nphi+ip];}
    ...
    { shtns_cfg c=make_cfg(lmax,lmax,1,sht_orthonormal,sht_gauss,nlat,nphi,SHT_PHI_CONTIGUOUS);cplx *v=calloc(nsp,sizeof(*v)),*q=calloc(c->nlm_cplx,sizeof(*q));fill_spatial_complex(v,nsp,0.31);analysis_spat_cplx_to_SH(c,v,nsp,q);for(int l=0;l<=lmax;++l)for(int mm=-l;mm<0;++mm)if((-mm)&1)q[LM_cplx(c,l,mm)]=-q[LM_cplx(c,l,mm)];
